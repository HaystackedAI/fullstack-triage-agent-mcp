...
BACKEND_URL = os.getenv("BACKEND_URL", "https://haystackedai.fastapicloud.dev")
...
def list_tasks() -> str:
    ...
    try:
        response = httpx.get(f"{BACKEND_URL}/api/tasks", timeout=10.0)
        response.raise_for_status()
        data = response.json()
        tasks = data["tasks"]
...
def delete_task(task_id: int) -> str:
    ...
    try:
        response = httpx.delete(f"{BACKEND_URL}/api/tasks/{task_id}", timeout=10.0)
        response.raise_for_status()
        data = response.json()
        task = data["task"]
